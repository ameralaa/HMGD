...
"""
Tully-Fisher Simulation Engine for HMGD
...
def calculate_hmgd_velocity(mass, r):
    L_h = 1.37e26 # Hubble Radius
    a_0 = (c**2) / (2 * math.pi * L_h)
    v2_newton = (G * mass) / r
    v2_flat = math.sqrt(G * mass * a_0)
...
def generate_tully_fisher_plot():
    ...
    for m in masses:
        ...
        v = calculate_hmgd_velocity(m, 50 * KPC)
    ...
    plt.loglog(masses/SOLAR_MASS, v_4, color='#79c0ff', linewidth=3, label='HMGD Prediction')
...
def generate_rotation_curves():
    ...
    for m, label, color in zip(galaxy_masses, labels, colors):
        v_vals = [calculate_hmgd_velocity(m, r_kpc * KPC) / 1000 for r_kpc in radii_kpc]
        plt.plot(radii_kpc, v_vals, label=label, color=color, linewidth=2)

    plt.title('HMGD Galactic Rotation Curves: The Flatness Proof', color='white', fontsize=14)
